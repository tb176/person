...
public class DateOperator extends DateParser {
	 ...
	 * @return
	 */
	public static String formatCompareDate(Date startDate,Date endDate){
		   ...
		   if(day > 0){
			   dateString.append( day+"天");
		   }
		   if(hour > 0){
			   dateString.append( hour+"小时");
		   }
		   if(minute > 0){
			   dateString.append( minute+"分钟");
		   }
		   if(second > 0){
			   dateString.append( second+"秒");
		   }
		return dateString.toString();
	}
	/**
	 * 获取两个日期相差时间（短输出),如果相差X天则仅返回 X天前，如果仅相差X小时则仅返回 X小时差，相差X秒，则返回X秒前
	 * @param startDate
	 * @param endDate
	 * @return
	 */
	public static String formatCompareDateShort(Date startDate,Date endDate){
		   String dateString = "";
		   long between=(endDate.getTime()-startDate.getTime())/1000;//除以1000是为了转换成秒
